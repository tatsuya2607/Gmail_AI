...
- **受信トレイ** — スター・未読フィルター・検索・ページネーション対応
- **ラベル管理** — 送信者ドメイン・メールアドレスでルールベースの自動ラベル付け
- **ゴミ箱** — ローカルゴミ箱（即時非表示）+ Gmail 側へのtrash同期
...
- **スレッド維持** — `In-Reply-To` / `References` ヘッダを自動設定
- **SQLite キャッシュ** — メール一覧を 3 分間キャッシュしてロード時間を短縮
- **ダーク / ライトモード** — システム設定連動 + 手動切替
...
│   ├── _components/HomeHeader.tsx            # ダッシュボード用ヘッダー（設定モーダル含む）
│   ├── account/[id]/
│   │   ├── page.tsx                          # 受信トレイ（サーバーコンポーネント）
│   │   ├── inbox.css
│   │   └── _components/
│   │       ├── InboxClient.tsx               # 受信トレイのメインUI
│   │       ├── InboxSidebar.tsx              # サイドバー（ビュー切替・ラベル）
│   │       ├── MessageList.tsx               # メール一覧
│   │       ├── EmailRow.tsx                  # メール行コンポーネント
│   │       ├── ComposeModal.tsx              # 新規メール作成モーダル
│   │       └── LabelModal.tsx                # ラベル管理モーダル
│   ├── compose/[messageId]/
│   │   ├── page.tsx                          # 返信作成画面（サーバーコンポーネント）
│   │   ├── compose.css
│   │   └── _components/
│   │       ├── ComposeClient.tsx             # 返信作成のメインUI
│   │       ├── SourcePane.tsx                # 元メール表示パネル
│   │       ├── GeneratePane.tsx              # AI生成・編集パネル
│   │       ├── ComposeToolbar.tsx            # ツールバー
│   │       └── SettingsModal.tsx             # 設定モーダル
│   ├── templates/
│   │   ├── page.tsx
│   │   ├── templates.css
│   │   └── _components/TemplatesClient.tsx
│   ├── shared.css                            # 共通CSS変数・テーマ
│   └── api/
│       ├── auth/google/route.ts              # OAuth 開始
│       ├── auth/google/callback/route.ts     # OAuth コールバック
│       ├── accounts/route.ts                 # アカウント一覧・削除
│       ├── messages/route.ts                 # メール一覧（キャッシュ対応）
│       ├── messages/[id]/route.ts            # メール詳細
│       ├── messages/[id]/read/route.ts       # 既読マーク
│       ├── messages/[id]/star/route.ts       # スター操作
│       ├── messages/[id]/delete/route.ts     # ゴミ箱移動 / 完全削除
│       ├── messages/[id]/restore/route.ts    # ゴミ箱から復元
│       ├── messages/invalidate/route.ts      # キャッシュ無効化
│       ├── labels/route.ts                   # ラベルCRUD
│       ├── labels/[id]/rules/route.ts        # ラベルルールCRUD
│       ├── reply/route.ts                    # AI 返信生成
│       ├── send/route.ts                     # メール送信
│       └── templates/route.ts               # テンプレートCRUD
├── components/
│   ├── AppHeader.tsx                         # 共通ヘッダー（パスブレッドクラム）
│   ├── AppFooter.tsx                         # 共通フッター
│   └── PageToolbar.tsx                       # 共通ツールバー
└── lib/
    ├── db.ts                                 # SQLite 初期化・スキーマ
